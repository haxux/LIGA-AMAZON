...
- [x] 0.1 Enable Docker Desktop's WSL integration for this distro, then confirm
...
- [x] 0.2 `docker compose up -d` and confirm `docker compose exec app php artisan test`
...
- [x] 1.1 RED — Create `tests/Feature/PanelAccessTest.php` with three tests:
...
- [x] 1.2 GREEN — `app/Models/User.php`: `implements FilamentUser`, add
      ...
      anyway.
- [x] 1.3 Verify — full suite green; re-run `PanelAccessTest` and confirm all three pass.

## Phase 2: Upload hardening — the stored-XSS blocker (TDD)

- [x] 2.1 RED — Create `tests/Feature/UploadValidationTest.php`: for **both**
...
- [x] 2.2 GREEN — `app/Filament/Resources/Teams/Schemas/TeamForm.php:37` and
...
- [x] 2.3 Verify — full suite green, including the pre-existing crest-upload happy path,
      which must still pass unchanged.

## Phase 3: Headers, HTTPS switch, throttle, robots

- [x] 3.1 RED — Create `tests/Feature/SecurityHeadersTest.php`: a public page response
      ...
      header tests fail.
- [x] 3.2 GREEN — Create `app/Http/Middleware/SecurityHeaders.php` emitting
      `X-Content-Type-Options: nosniff`, `X-Frame-Options: SAMEORIGIN`,
      `Referrer-Policy: strict-origin-when-cross-origin`,
      `Permissions-Policy: camera=(), microphone=(), geolocation=(), interest-cohort=()`,
      and HSTS **guarded by `$request->secure()`** (design D3).
- [x] 3.3 `bootstrap/app.php`: `$middleware->append(SecurityHeaders::class)`.
      **CONSTRAINT: global stack, not `$middleware->web(append: ...)`** — Filament's panel
      ...
      without headers.
- [x] 3.4 `config/app.php`: add `'force_https' => (bool) env('APP_FORCE_HTTPS', false)`.
...
- [x] 3.5 Add a test asserting `force_https` defaults to false and that enabling it makes
      `url()` generate an `https` scheme.
- [x] 3.6 `routes/web.php`: wrap the five public routes in
...
- [x] 3.7 `public/robots.txt`: `Disallow: /admin`.
- [x] 3.8 Verify — full suite green. Regression-check `StandingsPageTest`, `FixturesPageTest`,
...
- [x] 4.1 Create `.env.production.example`: `APP_ENV=production`, `APP_DEBUG=false`,
...
- [x] 4.2 Add a test asserting `.env.production.example` exists, sets `APP_DEBUG=false` and
...
- [x] 4.3 Create `DESPLIEGUE.md`: ordered deploy steps (key generation, `migrate --force`
...
- [x] 4.3b In the same document, a second section **"Pendiente — trabajo ya decidido"** for
...
- [x] 4.4 **Verification task**: cross-read `proposal.md`'s two "Out of Scope" lists against
      ...
      to prevent.
- [x] 4.5 `ARQUITECTURA.md` §8: add "Fase 7 — Producción" to the roadmap; mark Fase 6 done.

## Phase 5: Dependency advisories

- [x] 5.1 `composer update league/commonmark` — must land ≥ 2.10.0, clearing all four
...
- [x] 5.2 `composer audit` — must report zero advisories. Paste the actual output into
      `apply-progress.md`; do not assert it from memory.
- [~] 5.3 **PARTIAL — npm audit blocked externally.** `npm install` to rebuild `package-lock.json`, then `npm audit`. The audit
      ...
      **Outcome**: the lockfile half is DONE and was worse than expected — 60 of 106 entries
      had neither `resolved` nor `integrity` (built from a pre-populated `node_modules`, so
      `npm ci` could verify nothing); regenerated, all 107 now carry both, at the cost of 10
      dev/transitive versions moving inside their semver ranges (`npm run build` + full suite
      re-run green). The audit half is NOT done and is not doable here: npm 10's `quick`
      endpoint is retired (400) and npm 11's `bulk` endpoint returns `503 "We are currently
      performing maintenance"`. Confirmed external, not a project defect, by reproducing the
      identical failure in a clean scratch project with a valid name and version. Recorded as
      a pre-deploy retry in `DESPLIEGUE.md` §3.9.
- [x] 5.4 Full suite green after the update, plus `npm run build`, since 5.3 touches the
      asset toolchain.
- [x] 5.5 Add `composer audit` and `npm audit` to `DESPLIEGUE.md`'s pre-deploy steps (task
...
- [x] 6.1 `docker compose exec app php artisan test` — target **169 existing tests passing
...
- [x] 6.2 `./vendor/bin/pint --test` — PSR-12. The one issue introduced by this branch was
      fixed (commit `c6698b2`). Four issues remain in `bootstrap/providers.php`,
      `DivisionFactory`, `NewsFactory` and `SchemaMigrationTest` — **confirmed pre-existing
      by running pint on `main`** (137 files, 4 issues) and left alone rather than widening
      this diff.
- [x] 6.3 `npm run build` — asset build succeeds.
- [x] 6.4 Manual check in the browser: log into `/admin`, upload a `.svg` as a team crest
      ...
      **Confirmed by the owner**: the SVG upload raises the validation error.
- [x] 6.5 Confirm via `git diff --stat` that nothing under `database/` was touched —
      migrations, seeders and factories all unchanged — and that no database was re-seeded.
- [x] 6.6 Confirm `DESPLIEGUE.md` records the two deferred items the owner flagged for the
